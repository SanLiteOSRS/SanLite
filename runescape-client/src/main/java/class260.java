					...
					if (var0 == ScriptOpcodes.SETFOLLOWEROPSLOWPRIORITY) { // L: 3957
						...
						if (var0 == ScriptOpcodes.NC_PARAM) { // L: 3961
							...
							if (var5.isString()) { // L: 3992
								Interpreter.Interpreter_stringStack[++VarbitComposition.Interpreter_stringStackSize - 1] = UserComparator3.ItemComposition_get(var3).getStringParam(var4, var5.defaultStr); // L: 3993
							} else {
								Interpreter.Interpreter_intStack[++Interpreter.Interpreter_intStackSize - 1] = UserComparator3.ItemComposition_get(var3).getIntParam(var4, var5.defaultInt); // L: 3996
							}
