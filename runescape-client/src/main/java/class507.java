import net.runelite.mapping.Export;
import net.runelite.mapping.ObfuscatedGetter;
import net.runelite.mapping.ObfuscatedName;
import net.runelite.mapping.ObfuscatedSignature;

@ObfuscatedName("tn")
public class class507 extends DualNode {
	@ObfuscatedName("aj")
	@ObfuscatedSignature(
		descriptor = "Llh;"
	)
	@Export("DBRowType_cache")
	static EvictingDualNodeHashTable DBRowType_cache;
	@ObfuscatedName("ac")
	Object[][] field5050;
	@ObfuscatedName("aw")
	int[][] field5048;
	@ObfuscatedName("ay")
	@ObfuscatedGetter(
		intValue = 1590722671
	)
	public int field5049;

	static {
		DBRowType_cache = new EvictingDualNodeHashTable(64); // L: 11
	}

	class507() {
		this.field5049 = -1; // L: 14
	} // L: 17

	@ObfuscatedName("ac")
	@ObfuscatedSignature(
		descriptor = "(Ltl;B)V",
		garbageValue = "0"
	)
	void method8956(Buffer var1) {
		while (true) {
			int var2 = var1.readUnsignedByte(); // L: 36
			if (var2 == 0) { // L: 37
				return; // L: 40
			}

			this.method8957(var1, var2); // L: 38
		}
	}

	@ObfuscatedName("aw")
	@ObfuscatedSignature(
		descriptor = "(II)[Ljava/lang/Object;",
		garbageValue = "1435504572"
	)
	public Object[] method8969(int var1) {
		return this.field5050 == null ? null : this.field5050[var1]; // L: 43 44
	}

	@ObfuscatedName("ay")
	@ObfuscatedSignature(
		descriptor = "(Ltl;II)V",
		garbageValue = "-1573597306"
	)
	void method8957(Buffer var1, int var2) {
		if (var2 == 3) { // L: 48
			int var3 = var1.readUnsignedByte(); // L: 49
			if (this.field5050 == null) { // L: 50
				this.field5050 = new Object[var3][]; // L: 51
				this.field5048 = new int[var3][]; // L: 52
			}

			for (int var4 = var1.readUnsignedByte(); var4 != 255; var4 = var1.readUnsignedByte()) { // L: 54 55 78
				int var5 = var1.readUnsignedByte(); // L: 56
				int[] var6 = new int[var5]; // L: 57

				for (int var7 = 0; var7 < var5; ++var7) { // L: 58
					var6[var7] = var1.readUShortSmart(); // L: 59
				}

				Object[][] var16 = this.field5050; // L: 61
				int var10 = var1.readUShortSmart(); // L: 65
				Object[] var11 = new Object[var6.length * var10]; // L: 66

				for (int var12 = 0; var12 < var10; ++var12) { // L: 67
					for (int var13 = 0; var13 < var6.length; ++var13) { // L: 68
						int var14 = var13 + var6.length * var12; // L: 69
						class502 var15 = class11.method129(var6[var13]); // L: 70
						var11[var14] = var15.method8895(var1); // L: 71
					}
				}

				var16[var4] = var11; // L: 76
				this.field5048[var4] = var6; // L: 77
			}
		} else if (var2 == 4) { // L: 81
			this.field5049 = var1.method9079(); // L: 82
		}

	} // L: 85

	@ObfuscatedName("ap")
	@ObfuscatedSignature(
		descriptor = "(B)V",
		garbageValue = "0"
	)
	void method8958() {
	} // L: 87
}
