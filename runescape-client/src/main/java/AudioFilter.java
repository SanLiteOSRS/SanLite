...
public class AudioFilter {
	...
	AudioFilter() {
		...
		this.field430 = new int[2][2][4]; // L: 10
		this.field428 = new int[2][2][4]; // L: 11
		this.field432 = new int[2]; // L: 12
	} // L: 21

	@ObfuscatedName("f")
	float method1125(int var1, int var2, float var3) {
		float var4 = (float)this.field428[var1][0][var2] + var3 * (float)(this.field428[var1][1][var2] - this.field428[var1][0][var2]); // L: 24
	...
	float method1129(int var1, int var2, float var3) {
		float var4 = (float)this.field430[var1][0][var2] + var3 * (float)(this.field430[var1][1][var2] - this.field430[var1][0][var2]); // L: 35
	...
	int compute(int var1, float var2) {
		...
		if (this.pairs[var1] == 0) { // L: 47
			...
			var3 = this.method1125(var1, 0, var2); // L: 48
			field433[var1][0] = -2.0F * var3 * (float)Math.cos((double)this.method1129(var1, 0, var2)); // L: 49
			field433[var1][1] = var3 * var3; // L: 50

			float[] var10000;
			int var4;
			for (var4 = 1; var4 < this.pairs[var1]; ++var4) { // L: 51
				var3 = this.method1125(var1, var4, var2); // L: 52
				float var5 = -2.0F * var3 * (float)Math.cos((double)this.method1129(var1, var4, var2)); // L: 53
		...
		if (var3 != 0) { // L: 70
			...
			for (var5 = 0; var5 < 2; ++var5) { // L: 74
				for (var6 = 0; var6 < this.pairs[var5]; ++var6) { // L: 75
					this.field430[var5][0][var6] = var1.readUnsignedShort(); // L: 76
					this.field428[var5][0][var6] = var1.readUnsignedShort(); // L: 77
				}
			}
			...
			for (var5 = 0; var5 < 2; ++var5) { // L: 80
				for (var6 = 0; var6 < this.pairs[var5]; ++var6) { // L: 81
					if ((var7 & 1 << var5 * 4 << var6) != 0) { // L: 82
						this.field430[var5][1][var6] = var1.readUnsignedShort(); // L: 83
						this.field428[var5][1][var6] = var1.readUnsignedShort(); // L: 84
					} else {
						this.field430[var5][1][var6] = this.field430[var5][0][var6]; // L: 87
						this.field428[var5][1][var6] = this.field428[var5][0][var6]; // L: 88
					}
				}
			}
