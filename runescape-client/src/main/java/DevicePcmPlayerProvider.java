			...
			while (true) {
				if (!KitDefinition.isKeyDown()) {
					...
					if (Login.hoveredWorldIndex != -1) {
						...
						if (Login.worldSelectPage < Login.worldSelectPagesCount && FriendsChatManager.worldSelectRightSprite != null && MouseHandler.MouseHandler_lastPressedX >= InvDefinition.canvasWidth - FriendsChatManager.worldSelectRightSprite.subWidth - 5 && MouseHandler.MouseHandler_lastPressedX <= InvDefinition.canvasWidth && MouseHandler.MouseHandler_lastPressedY >= GameEngine.canvasHeight / 2 - 50 && MouseHandler.MouseHandler_lastPressedY <= GameEngine.canvasHeight / 2 + 50) {
							++Login.worldSelectPage;
						}
					}
					break;
				...
				} else if (SpriteMask.field2994 == 97 && Login.worldSelectPage < Login.worldSelectPagesCount && FriendsChatManager.worldSelectRightSprite != null) {
					++Login.worldSelectPage;
				}
			}
