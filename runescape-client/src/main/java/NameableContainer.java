import java.util.Arrays;
import java.util.Comparator;
import java.util.HashMap;
import net.runelite.mapping.Export;
import net.runelite.mapping.Implements;
import net.runelite.mapping.ObfuscatedGetter;
import net.runelite.mapping.ObfuscatedName;
import net.runelite.mapping.ObfuscatedSignature;
import net.runelite.rs.ScriptOpcodes;

@ObfuscatedName("qt")
@Implements("NameableContainer")
public abstract class NameableContainer {
	@ObfuscatedName("aa")
	@ObfuscatedGetter(
		intValue = 605463463
	)
	@Export("capacity")
	final int capacity;
	@ObfuscatedName("af")
	@ObfuscatedGetter(
		intValue = 844754279
	)
	@Export("size")
	int size;
	@ObfuscatedName("ad")
	@ObfuscatedSignature(
		descriptor = "[Lqs;"
	)
	@Export("array")
	Nameable[] array;
	@ObfuscatedName("aq")
	@Export("usernamesMap")
	HashMap usernamesMap;
	@ObfuscatedName("al")
	@Export("previousUsernamesMap")
	HashMap previousUsernamesMap;
	@ObfuscatedName("an")
	@Export("comparator")
	Comparator comparator;

	NameableContainer(int var1) {
		this.size = 0; // L: 10
		this.comparator = null; // L: 14
		this.capacity = var1; // L: 17
		this.array = this.newTypedArray(var1); // L: 18
		this.usernamesMap = new HashMap(var1 / 8); // L: 19
		this.previousUsernamesMap = new HashMap(var1 / 8); // L: 20
	} // L: 21

	@ObfuscatedName("au")
	@ObfuscatedSignature(
		descriptor = "(I)Lqs;",
		garbageValue = "-1997275235"
	)
	@Export("newInstance")
	abstract Nameable newInstance();

	@ObfuscatedName("ae")
	@ObfuscatedSignature(
		descriptor = "(IB)[Lqs;",
		garbageValue = "99"
	)
	@Export("newTypedArray")
	abstract Nameable[] newTypedArray(int var1);

	@ObfuscatedName("bk")
	@ObfuscatedSignature(
		descriptor = "(I)V",
		garbageValue = "501302871"
	)
	@Export("clear")
	public void clear() {
		this.size = 0; // L: 24
		Arrays.fill(this.array, (Object)null); // L: 25
		this.usernamesMap.clear(); // L: 26
		this.previousUsernamesMap.clear(); // L: 27
	} // L: 28

	@ObfuscatedName("bv")
	@ObfuscatedSignature(
		descriptor = "(I)I",
		garbageValue = "899062224"
	)
	@Export("getSize")
	public int getSize() {
		return this.size; // L: 31
	}

	@ObfuscatedName("bt")
	@ObfuscatedSignature(
		descriptor = "(B)Z",
		garbageValue = "31"
	)
	@Export("isFull")
	public boolean isFull() {
		return this.capacity == this.size; // L: 35
	}

	@ObfuscatedName("bd")
	@ObfuscatedSignature(
		descriptor = "(Luc;I)Z",
		garbageValue = "-2059645334"
	)
	@Export("contains")
	public boolean contains(Username var1) {
		if (!var1.hasCleanName()) { // L: 39
			return false;
		} else {
			return this.usernamesMap.containsKey(var1) ? true : this.previousUsernamesMap.containsKey(var1); // L: 40 41
		}
	}

	@ObfuscatedName("by")
	@ObfuscatedSignature(
		descriptor = "(Luc;I)Lqs;",
		garbageValue = "1337477510"
	)
	@Export("getByUsername")
	public Nameable getByUsername(Username var1) {
		Nameable var2 = this.getByCurrentUsername(var1); // L: 45
		return var2 != null ? var2 : this.getByPreviousUsername(var1); // L: 46 47
	}

	@ObfuscatedName("bs")
	@ObfuscatedSignature(
		descriptor = "(Luc;I)Lqs;",
		garbageValue = "1463265876"
	)
	@Export("getByCurrentUsername")
	Nameable getByCurrentUsername(Username var1) {
		return !var1.hasCleanName() ? null : (Nameable)this.usernamesMap.get(var1); // L: 51 52
	}

	@ObfuscatedName("bm")
	@ObfuscatedSignature(
		descriptor = "(Luc;S)Lqs;",
		garbageValue = "24556"
	)
	@Export("getByPreviousUsername")
	Nameable getByPreviousUsername(Username var1) {
		return !var1.hasCleanName() ? null : (Nameable)this.previousUsernamesMap.get(var1); // L: 56 57
	}

	@ObfuscatedName("bf")
	@ObfuscatedSignature(
		descriptor = "(Luc;I)Z",
		garbageValue = "-810573248"
	)
	@Export("removeByUsername")
	public final boolean removeByUsername(Username var1) {
		Nameable var2 = this.getByCurrentUsername(var1); // L: 61
		if (var2 == null) { // L: 62
			return false;
		} else {
			this.remove(var2); // L: 63
			return true; // L: 64
		}
	}

	@ObfuscatedName("bq")
	@ObfuscatedSignature(
		descriptor = "(Lqs;I)V",
		garbageValue = "1938550187"
	)
	@Export("remove")
	final void remove(Nameable var1) {
		int var2 = this.indexOf(var1); // L: 68
		if (var2 != -1) { // L: 69
			this.arrayRemove(var2); // L: 70
			this.mapRemove(var1); // L: 71
		}
	} // L: 72

	@ObfuscatedName("ba")
	@ObfuscatedSignature(
		descriptor = "(Luc;B)Lqs;",
		garbageValue = "118"
	)
	@Export("addLastNoPreviousUsername")
	Nameable addLastNoPreviousUsername(Username var1) {
		return this.addLast(var1, (Username)null); // L: 75
	}

	@ObfuscatedName("bl")
	@ObfuscatedSignature(
		descriptor = "(Luc;Luc;B)Lqs;",
		garbageValue = "0"
	)
	@Export("addLast")
	Nameable addLast(Username var1, Username var2) {
		if (this.getByCurrentUsername(var1) != null) { // L: 79
			throw new IllegalStateException();
		} else {
			Nameable var3 = this.newInstance(); // L: 80
			var3.set(var1, var2); // L: 81
			this.arrayAddLast(var3); // L: 82
			this.mapPut(var3); // L: 83
			return var3; // L: 84
		}
	}

	@ObfuscatedName("bp")
	@ObfuscatedSignature(
		descriptor = "(IB)Lqs;",
		garbageValue = "20"
	)
	@Export("get")
	public final Nameable get(int var1) {
		if (var1 >= 0 && var1 < this.size) { // L: 88
			return this.array[var1];
		} else {
			throw new ArrayIndexOutOfBoundsException(var1); // L: 89
		}
	}

	@ObfuscatedName("bu")
	@ObfuscatedSignature(
		descriptor = "(I)V",
		garbageValue = "1136384662"
	)
	@Export("sort")
	public final void sort() {
		if (this.comparator == null) { // L: 93
			Arrays.sort(this.array, 0, this.size); // L: 94
		} else {
			Arrays.sort(this.array, 0, this.size, this.comparator); // L: 97
		}

	} // L: 99

	@ObfuscatedName("bo")
	@ObfuscatedSignature(
		descriptor = "(Lqs;Luc;Luc;I)V",
		garbageValue = "1687018867"
	)
	@Export("changeName")
	final void changeName(Nameable var1, Username var2, Username var3) {
		this.mapRemove(var1); // L: 102
		var1.set(var2, var3); // L: 103
		this.mapPut(var1); // L: 104
	} // L: 105

	@ObfuscatedName("bb")
	@ObfuscatedSignature(
		descriptor = "(Lqs;B)I",
		garbageValue = "2"
	)
	@Export("indexOf")
	final int indexOf(Nameable var1) {
		for (int var2 = 0; var2 < this.size; ++var2) { // L: 108
			if (this.array[var2] == var1) { // L: 109
				return var2;
			}
		}

		return -1; // L: 111
	}

	@ObfuscatedName("br")
	@ObfuscatedSignature(
		descriptor = "(Lqs;I)V",
		garbageValue = "-1136381607"
	)
	@Export("mapRemove")
	final void mapRemove(Nameable var1) {
		if (this.usernamesMap.remove(var1.username) == null) { // L: 115
			throw new IllegalStateException();
		} else {
			if (var1.previousUsername != null) {
				this.previousUsernamesMap.remove(var1.previousUsername); // L: 116
			}

		}
	} // L: 117

	@ObfuscatedName("be")
	@ObfuscatedSignature(
		descriptor = "(Lqs;I)V",
		garbageValue = "-633410463"
	)
	@Export("arrayAddLast")
	final void arrayAddLast(Nameable var1) {
		this.array[++this.size - 1] = var1; // L: 120
	} // L: 121

	@ObfuscatedName("bi")
	@ObfuscatedSignature(
		descriptor = "(Lqs;B)V",
		garbageValue = "-26"
	)
	@Export("mapPut")
	final void mapPut(Nameable var1) {
		this.usernamesMap.put(var1.username, var1); // L: 124
		if (var1.previousUsername != null) { // L: 125
			Nameable var2 = (Nameable)this.previousUsernamesMap.put(var1.previousUsername, var1); // L: 126
			if (var2 != null && var2 != var1) { // L: 127
				var2.previousUsername = null; // L: 128
			}
		}

	} // L: 131

	@ObfuscatedName("bz")
	@ObfuscatedSignature(
		descriptor = "(IB)V",
		garbageValue = "-87"
	)
	@Export("arrayRemove")
	final void arrayRemove(int var1) {
		--this.size; // L: 134
		if (var1 < this.size) { // L: 135
			System.arraycopy(this.array, var1 + 1, this.array, var1, this.size - var1); // L: 136
		}

	} // L: 138

	@ObfuscatedName("bx")
	@ObfuscatedSignature(
		descriptor = "(B)V",
		garbageValue = "85"
	)
	@Export("removeComparator")
	public final void removeComparator() {
		this.comparator = null; // L: 144
	} // L: 145

	@ObfuscatedName("bn")
	@ObfuscatedSignature(
		descriptor = "(Ljava/util/Comparator;B)V",
		garbageValue = "68"
	)
	@Export("addComparator")
	public final void addComparator(Comparator var1) {
		if (this.comparator == null) { // L: 148
			this.comparator = var1; // L: 149
		} else if (this.comparator instanceof AbstractUserComparator) { // L: 151
			((AbstractUserComparator)this.comparator).addComparator(var1); // L: 152
		}

	} // L: 154

	@ObfuscatedName("bj")
	@ObfuscatedSignature(
		descriptor = "(ILdh;ZI)I",
		garbageValue = "1913527870"
	)
	static int method7990(int var0, Script var1, boolean var2) {
		int var3;
		int var6;
		int var9;
		if (var0 == ScriptOpcodes.ENUM_STRING) { // L: 2813
			SoundCache.Interpreter_intStackSize -= 2; // L: 2814
			var3 = Interpreter.Interpreter_intStack[SoundCache.Interpreter_intStackSize]; // L: 2815
			var9 = Interpreter.Interpreter_intStack[SoundCache.Interpreter_intStackSize + 1]; // L: 2816
			EnumComposition var10 = WorldMapID.getEnum(var3); // L: 2817
			if (var10.outputType != 's') { // L: 2818
			}

			for (var6 = 0; var6 < var10.outputCount; ++var6) { // L: 2819
				if (var9 == var10.keys[var6]) { // L: 2820
					Interpreter.Interpreter_stringStack[++class137.Interpreter_stringStackSize - 1] = var10.strVals[var6]; // L: 2821
					var10 = null; // L: 2822
					break;
				}
			}

			if (var10 != null) { // L: 2826
				Interpreter.Interpreter_stringStack[++class137.Interpreter_stringStackSize - 1] = var10.defaultStr;
			}

			return 1; // L: 2827
		} else if (var0 != ScriptOpcodes.ENUM) { // L: 2829
			if (var0 == ScriptOpcodes.ENUM_GETOUTPUTCOUNT) { // L: 2855
				var3 = Interpreter.Interpreter_intStack[--SoundCache.Interpreter_intStackSize]; // L: 2856
				EnumComposition var4 = WorldMapID.getEnum(var3); // L: 2857
				Interpreter.Interpreter_intStack[++SoundCache.Interpreter_intStackSize - 1] = var4.size(); // L: 2858
				return 1; // L: 2859
			} else {
				return 2; // L: 2861
			}
		} else {
			SoundCache.Interpreter_intStackSize -= 4; // L: 2830
			var3 = Interpreter.Interpreter_intStack[SoundCache.Interpreter_intStackSize]; // L: 2831
			var9 = Interpreter.Interpreter_intStack[SoundCache.Interpreter_intStackSize + 1]; // L: 2832
			int var5 = Interpreter.Interpreter_intStack[SoundCache.Interpreter_intStackSize + 2]; // L: 2833
			var6 = Interpreter.Interpreter_intStack[SoundCache.Interpreter_intStackSize + 3]; // L: 2834
			EnumComposition var7 = WorldMapID.getEnum(var5); // L: 2835
			if (var3 == var7.inputType && var9 == var7.outputType) { // L: 2836
				for (int var8 = 0; var8 < var7.outputCount; ++var8) { // L: 2841
					if (var6 == var7.keys[var8]) { // L: 2842
						if (var9 == 115) { // L: 2843
							Interpreter.Interpreter_stringStack[++class137.Interpreter_stringStackSize - 1] = var7.strVals[var8];
						} else {
							Interpreter.Interpreter_intStack[++SoundCache.Interpreter_intStackSize - 1] = var7.intVals[var8]; // L: 2844
						}

						var7 = null; // L: 2845
						break;
					}
				}

				if (var7 != null) { // L: 2849
					if (var9 == 115) { // L: 2850
						Interpreter.Interpreter_stringStack[++class137.Interpreter_stringStackSize - 1] = var7.defaultStr;
					} else {
						Interpreter.Interpreter_intStack[++SoundCache.Interpreter_intStackSize - 1] = var7.defaultInt; // L: 2851
					}
				}

				return 1; // L: 2853
			} else {
				if (var9 == 115) { // L: 2837
					Interpreter.Interpreter_stringStack[++class137.Interpreter_stringStackSize - 1] = "null";
				} else {
					Interpreter.Interpreter_intStack[++SoundCache.Interpreter_intStackSize - 1] = 0; // L: 2838
				}

				return 1; // L: 2839
			}
		}
	}

	@ObfuscatedName("kh")
	@ObfuscatedSignature(
		descriptor = "(I)Z",
		garbageValue = "-587387192"
	)
	static final boolean method7991() {
		return Client.isMenuOpen; // L: 9211
	}
}
