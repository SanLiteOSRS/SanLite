import net.runelite.mapping.ObfuscatedName;
import net.runelite.mapping.ObfuscatedSignature;

@ObfuscatedName("ql")
public abstract class class426 extends class269 implements class485 {
	@ObfuscatedSignature(
		descriptor = "(Lml;Lnq;I)V"
	)
	protected class426(StudioGame var1, Language var2, int var3) {
		super(var1, var2, var3); // L: 15
	} // L: 16

	@ObfuscatedName("aj")
	@ObfuscatedSignature(
		descriptor = "(II)Lqd;",
		garbageValue = "-1357061563"
	)
	protected abstract class428 vmethod8424(int var1);

	@ObfuscatedName("al")
	@ObfuscatedSignature(
		descriptor = "(B)I",
		garbageValue = "91"
	)
	public int method7938() {
		return super.field3035; // L: 19
	}

	@ObfuscatedName("ac")
	@ObfuscatedSignature(
		descriptor = "(IB)Ljava/lang/Object;",
		garbageValue = "-118"
	)
	public Object vmethod8946(int var1) {
		class428 var2 = this.vmethod8424(var1); // L: 24
		return var2 != null && var2.method7953() ? var2.method7958() : null; // L: 25 26
	}

	@ObfuscatedName("ab")
	@ObfuscatedSignature(
		descriptor = "(Lsy;I)Lsj;",
		garbageValue = "295942057"
	)
	public class486 method7939(Buffer var1) {
		int var2 = var1.readUnsignedShort(); // L: 30
		class428 var3 = this.vmethod8424(var2); // L: 31
		class486 var4 = new class486(var2); // L: 32
		Class var5 = var3.field4637.field4858; // L: 33
		if (var5 == Integer.class) { // L: 34
			var4.field4966 = var1.readInt();
		} else if (var5 == Long.class) {
			var4.field4966 = var1.readLong(); // L: 35
		} else if (var5 == String.class) { // L: 36
			var4.field4966 = var1.readStringCp1252NullCircumfixed();
		} else {
			if (!class481.class.isAssignableFrom(var5)) { // L: 37
				throw new IllegalStateException(); // L: 46
			}

			try {
				class481 var6 = (class481)var5.newInstance(); // L: 39
				var6.method8920(var1); // L: 40
				var4.field4966 = var6; // L: 41
			} catch (InstantiationException var7) { // L: 43
			} catch (IllegalAccessException var8) { // L: 44
			}
		}

		return var4; // L: 47
	}
}
