...
public class HealthBar extends Node {
	...
	static void load() {
		...
		if (Client.titleLoadingStage == 0) {
			...
			class3.sceneMinimapSprite = new SpritePixels(512, 512);
			...
			if (Client.titleLoadingStage == 45) {
				...
				if (Client.titleLoadingStage == 50) {
					...
					if (!Clock.archive2.isFullyLoaded()) {
						...
						ObjectComposition.ObjectDefinition_isLowDetail = var33;
					...
					if (GrandExchangeOfferNameComparator.compass == null) {
						GrandExchangeOfferNameComparator.compass = GameEngine.SpriteBuffer_getSprite(GrandExchangeEvent.archive8, UserComparator10.spriteIds.compass, 0);
					} else {
						++var0;
					}

					if (UrlRequester.redHintArrowSprite == null) {
						UrlRequester.redHintArrowSprite = GameEngine.SpriteBuffer_getSprite(GrandExchangeEvent.archive8, UserComparator10.spriteIds.field3825, 0);
					} else {
						++var0;
					}
	...
	static final void worldToScreen(int var0, int var1, int var2) {
		if (var0 >= 128 && var1 >= 128 && var0 <= 13056 && var1 <= 13056) {
			int var3 = Language.getTileHeight(var0, var1, ItemLayer.Client_plane) - var2;
			var0 -= MouseHandler.cameraX;
			var3 -= WorldMapSection3.cameraY;
			var1 -= ObjectSound.cameraZ;
			int var4 = Rasterizer3D.Rasterizer3D_sine[SceneTilePaint.cameraPitch];
			int var5 = Rasterizer3D.Rasterizer3D_cosine[SceneTilePaint.cameraPitch];
