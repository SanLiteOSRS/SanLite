		...
		if (var0 == ScriptOpcodes.FRIEND_COUNT) { // L: 2351
			...
			if (var0 == ScriptOpcodes.FRIEND_GETNAME) {
				...
				if (var0 == ScriptOpcodes.FRIEND_SETRANK) {
					...
					if (Huffman.friendsChatManager != null) { // L: 2419
						Interpreter.Interpreter_stringStack[++Interpreter.Interpreter_stringStackSize - 1] = Huffman.friendsChatManager.name; // L: 2420
					} else {
					...
					if (Huffman.friendsChatManager != null) { // L: 2426
						Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Huffman.friendsChatManager.getSize(); // L: 2427
					} else {
					...
					if (Huffman.friendsChatManager != null && var3 < Huffman.friendsChatManager.getSize()) { // L: 2434
						Interpreter.Interpreter_stringStack[++Interpreter.Interpreter_stringStackSize - 1] = Huffman.friendsChatManager.get(var3).getUsername().getName(); // L: 2435
					} else {
					...
					if (Huffman.friendsChatManager != null && var3 < Huffman.friendsChatManager.getSize()) { // L: 2442
						Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = ((Buddy)Huffman.friendsChatManager.get(var3)).getWorld(); // L: 2443
					} else {
					...
					if (Huffman.friendsChatManager != null && var3 < Huffman.friendsChatManager.getSize()) { // L: 2450
						Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = ((Buddy)Huffman.friendsChatManager.get(var3)).rank; // L: 2451
					} else {
					...
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Huffman.friendsChatManager != null ? Huffman.friendsChatManager.minKick : 0; // L: 2457
					return 1; // L: 2458
					...
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Huffman.friendsChatManager != null ? Huffman.friendsChatManager.rank : 0; // L: 2466
					return 1; // L: 2467
					...
					if (Huffman.friendsChatManager != null && var3 < Huffman.friendsChatManager.getSize() && Huffman.friendsChatManager.get(var3).getUsername().equals(class101.localPlayer.username)) { // L: 2504
					...
					if (Huffman.friendsChatManager != null && Huffman.friendsChatManager.owner != null) { // L: 2511
						Interpreter.Interpreter_stringStack[++Interpreter.Interpreter_stringStackSize - 1] = Huffman.friendsChatManager.owner; // L: 2512
					} else {
					...
					if (Huffman.friendsChatManager != null && var3 < Huffman.friendsChatManager.getSize() && ((FriendsChatMember)Huffman.friendsChatManager.get(var3)).isFriend()) { // L: 2519
					...
					if (var0 == 3628) {
						...
						if (var0 == 3629) {
							...
							if (Huffman.friendsChatManager != null) { // L: 2610
								Huffman.friendsChatManager.removeComparator(); // L: 2611
							}

							return 1; // L: 2613
						} else if (var0 == 3645) {
							var7 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize] == 1; // L: 2616
							if (Huffman.friendsChatManager != null) { // L: 2617
								Huffman.friendsChatManager.addComparator(new UserComparator1(var7)); // L: 2618
							}

							return 1; // L: 2620
						} else if (var0 == 3646) {
							var7 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize] == 1; // L: 2623
							if (Huffman.friendsChatManager != null) { // L: 2624
								Huffman.friendsChatManager.addComparator(new UserComparator2(var7)); // L: 2625
							}

							return 1; // L: 2627
						} else if (var0 == 3647) {
							var7 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize] == 1; // L: 2630
							if (Huffman.friendsChatManager != null) { // L: 2631
								Huffman.friendsChatManager.addComparator(new UserComparator3(var7)); // L: 2632
							}

							return 1; // L: 2634
						} else if (var0 == 3648) {
							var7 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize] == 1; // L: 2637
							if (Huffman.friendsChatManager != null) { // L: 2638
								Huffman.friendsChatManager.addComparator(new UserComparator4(var7)); // L: 2639
							}

							return 1; // L: 2641
						} else if (var0 == 3649) {
							var7 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize] == 1; // L: 2644
							if (Huffman.friendsChatManager != null) { // L: 2645
								Huffman.friendsChatManager.addComparator(new UserComparator5(var7)); // L: 2646
							}

							return 1; // L: 2648
						} else if (var0 == 3650) {
							var7 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize] == 1; // L: 2651
							if (Huffman.friendsChatManager != null) { // L: 2652
								Huffman.friendsChatManager.addComparator(new UserComparator6(var7)); // L: 2653
							}

							return 1; // L: 2655
						} else if (var0 == 3651) {
							var7 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize] == 1; // L: 2658
							if (Huffman.friendsChatManager != null) { // L: 2659
								Huffman.friendsChatManager.addComparator(new UserComparator7(var7)); // L: 2660
							}

							return 1; // L: 2662
						} else if (var0 == 3652) {
							var7 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize] == 1; // L: 2665
							if (Huffman.friendsChatManager != null) { // L: 2666
								Huffman.friendsChatManager.addComparator(new UserComparator8(var7)); // L: 2667
							}

							return 1; // L: 2669
						} else if (var0 == 3653) {
							var7 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize] == 1; // L: 2672
							if (Huffman.friendsChatManager != null) { // L: 2673
								Huffman.friendsChatManager.addComparator(new UserComparator9(var7)); // L: 2674
							}

							return 1; // L: 2676
						} else if (var0 == 3654) {
							var7 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize] == 1; // L: 2679
							if (Huffman.friendsChatManager != null) { // L: 2680
								Huffman.friendsChatManager.addComparator(new UserComparator10(var7)); // L: 2681
							}

							return 1; // L: 2683
						} else if (var0 == 3655) {
							if (Huffman.friendsChatManager != null) { // L: 2686
								Huffman.friendsChatManager.sort(); // L: 2687
							}
							...
							if (Huffman.friendsChatManager != null) { // L: 2698
								Huffman.friendsChatManager.addComparator(new BuddyRankComparator(var7)); // L: 2699
							}
					...
					if (Huffman.friendsChatManager != null && var3 < Huffman.friendsChatManager.getSize() && ((FriendsChatMember)Huffman.friendsChatManager.get(var3)).isIgnored()) { // L: 2527
