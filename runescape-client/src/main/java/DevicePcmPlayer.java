...
public class DevicePcmPlayer extends PcmPlayer {
	...
	@Export("init")
	protected void init() {
		this.format = new AudioFormat((float)SoundCache.field337, 16, class157.PcmPlayer_stereo ? 2 : 1, true, false); // L: 22
		this.byteSamples = new byte[256 << (class157.PcmPlayer_stereo ? 2 : 1)]; // L: 23
	} // L: 24
	...
	protected void open(int var1) throws LineUnavailableException {
		try {
			Info var2 = new Info(SourceDataLine.class, this.format, var1 << (class157.PcmPlayer_stereo ? 2 : 1)); // L: 29
			...
			if (Renderable.method4454(var1) != 1) { // L: 36
				this.open(class37.method712(var1)); // L: 37
			} else {
				this.line = null; // L: 40
				throw var3; // L: 41
			}
	...
	protected int position() {
		return this.capacity2 - (this.line.available() >> (class157.PcmPlayer_stereo ? 2 : 1)); // L: 47
	}
	...
	protected void write() {
		int var1 = 256; // L: 53
		if (class157.PcmPlayer_stereo) { // L: 54
			var1 <<= 1;
		}
