...
public final class KeyHandler implements KeyListener, FocusListener {
	...
	static final void drawEntities(int var0, int var1, int var2, int var3) {
		...
		for (Projectile var4 = (Projectile)Client.projectiles.last(); var4 != null; var4 = (Projectile)Client.projectiles.previous()) {
			if (var4.plane == ItemLayer.Client_plane && Client.cycle <= var4.cycleEnd) {
				if (Client.cycle >= var4.cycleStart) {
					...
					UserComparator3.scene.drawEntity(ItemLayer.Client_plane, (int)var4.x, (int)var4.y, (int)var4.z, 60, var4, var4.yaw, -1L, false);
				}
			} else {
				var4.remove();
			}
		...
		for (GraphicsObject var32 = (GraphicsObject)Client.graphicsObjects.last(); var32 != null; var32 = (GraphicsObject)Client.graphicsObjects.previous()) {
			if (var32.plane == ItemLayer.Client_plane && !var32.isFinished) {
			...
			if (NetSocket.clientPreferences.roofsHidden) {
				...
				label668: {
					...
					if (SceneTilePaint.cameraPitch < 310) {
						...
						if ((Tiles.Tiles_renderFlags[ItemLayer.Client_plane][var9][var10] & 4) != 0) {
							var6 = ItemLayer.Client_plane;
						}
					...
					if (class60.localPlayer.x >= 0 && class60.localPlayer.y >= 0 && class60.localPlayer.x < 13312 && class60.localPlayer.y < 13312) {
						if ((Tiles.Tiles_renderFlags[ItemLayer.Client_plane][class60.localPlayer.x >> 7][class60.localPlayer.y >> 7] & 4) != 0) {
							var6 = ItemLayer.Client_plane;
						}
		...
		var8 = SceneTilePaint.cameraPitch;
		...
		UserComparator3.scene.draw(MouseHandler.cameraX, WorldMapSection3.cameraY, ObjectSound.cameraZ, SceneTilePaint.cameraPitch, Canvas.cameraYaw, var28);
		...
		if (Client.hintArrowType == 2) {
			HealthBar.worldToScreen((Client.hintArrowX - class182.baseX * 64 << 7) + Client.hintArrowSubX * 4096, (Client.hintArrowY - SecureRandomFuture.baseY * 64 << 7) + Client.hintArrowSubY * 4096, Client.hintArrowHeight * 4);
