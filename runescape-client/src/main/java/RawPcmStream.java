import net.runelite.mapping.Export;
import net.runelite.mapping.Implements;
import net.runelite.mapping.ObfuscatedName;
import net.runelite.mapping.ObfuscatedSignature;

@ObfuscatedName("bk")
@Implements("RawPcmStream")
public class RawPcmStream extends PcmStream {
	@ObfuscatedName("h")
	int field466;
	@ObfuscatedName("c")
	int field453;
	@ObfuscatedName("o")
	int field460;
	@ObfuscatedName("g")
	int field455;
	@ObfuscatedName("l")
	int field463;
	@ObfuscatedName("z")
	int field457;
	@ObfuscatedName("t")
	int field452;
	@ObfuscatedName("v")
	@Export("numLoops")
	int numLoops;
	@ObfuscatedName("b")
	@Export("start")
	int start;
	@ObfuscatedName("q")
	@Export("end")
	int end;
	@ObfuscatedName("i")
	boolean field462;
	@ObfuscatedName("x")
	int field454;
	@ObfuscatedName("k")
	int field464;
	@ObfuscatedName("r")
	int field465;
	@ObfuscatedName("p")
	int field461;

	@ObfuscatedSignature(
		descriptor = "(Laf;III)V"
	)
	RawPcmStream(RawSound var1, int var2, int var3, int var4) {
		super.sound = var1;
		this.start = var1.start;
		this.end = var1.end;
		this.field462 = var1.field381;
		this.field453 = var2;
		this.field460 = var3;
		this.field455 = var4;
		this.field466 = 0;
		this.method886();
	}

	@ObfuscatedSignature(
		descriptor = "(Laf;II)V"
	)
	RawPcmStream(RawSound var1, int var2, int var3) {
		super.sound = var1;
		this.start = var1.start;
		this.end = var1.end;
		this.field462 = var1.field381;
		this.field453 = var2;
		this.field460 = var3;
		this.field455 = 8192;
		this.field466 = 0;
		this.method886();
	}

	@ObfuscatedName("l")
	void method886() {
		this.field463 = this.field460;
		this.field457 = method924(this.field460, this.field455);
		this.field452 = method880(this.field460, this.field455);
	}

	@ObfuscatedName("z")
	@ObfuscatedSignature(
		descriptor = "()Lbt;"
	)
	@Export("firstSubStream")
	protected PcmStream firstSubStream() {
		return null;
	}

	@ObfuscatedName("t")
	@ObfuscatedSignature(
		descriptor = "()Lbt;"
	)
	@Export("nextSubStream")
	protected PcmStream nextSubStream() {
		return null;
	}

	@ObfuscatedName("v")
	protected int vmethod4560() {
		return this.field460 == 0 && this.field454 == 0 ? 0 : 1;
	}

	@ObfuscatedName("b")
	@Export("fill")
	public synchronized void fill(int[] var1, int var2, int var3) {
		if (this.field460 == 0 && this.field454 == 0) {
			this.skip(var3);
		} else {
			RawSound var4 = (RawSound)super.sound;
			int var5 = this.start << 8;
			int var6 = this.end << 8;
			int var7 = var4.samples.length << 8;
			int var8 = var6 - var5;
			if (var8 <= 0) {
				this.numLoops = 0;
			}

			int var9 = var2;
			var3 += var2;
			if (this.field466 < 0) {
				if (this.field453 <= 0) {
					this.method917();
					this.remove();
					return;
				}

				this.field466 = 0;
			}

			if (this.field466 >= var7) {
				if (this.field453 >= 0) {
					this.method917();
					this.remove();
					return;
				}

				this.field466 = var7 - 1;
			}

			if (this.numLoops < 0) {
				if (this.field462) {
					if (this.field453 < 0) {
						var9 = this.method909(var1, var2, var5, var3, var4.samples[this.start]);
						if (this.field466 >= var5) {
							return;
						}

						this.field466 = var5 + var5 - 1 - this.field466;
						this.field453 = -this.field453;
					}

					while (true) {
						var9 = this.method908(var1, var9, var6, var3, var4.samples[this.end - 1]);
						if (this.field466 < var6) {
							return;
						}

						this.field466 = var6 + var6 - 1 - this.field466;
						this.field453 = -this.field453;
						var9 = this.method909(var1, var9, var5, var3, var4.samples[this.start]);
						if (this.field466 >= var5) {
							return;
						}

						this.field466 = var5 + var5 - 1 - this.field466;
						this.field453 = -this.field453;
					}
				} else if (this.field453 < 0) {
					while (true) {
						var9 = this.method909(var1, var9, var5, var3, var4.samples[this.end - 1]);
						if (this.field466 >= var5) {
							return;
						}

						this.field466 = var6 - 1 - (var6 - 1 - this.field466) % var8;
					}
				} else {
					while (true) {
						var9 = this.method908(var1, var9, var6, var3, var4.samples[this.start]);
						if (this.field466 < var6) {
							return;
						}

						this.field466 = var5 + (this.field466 - var5) % var8;
					}
				}
			} else {
				if (this.numLoops > 0) {
					if (this.field462) {
						label127: {
							if (this.field453 < 0) {
								var9 = this.method909(var1, var2, var5, var3, var4.samples[this.start]);
								if (this.field466 >= var5) {
									return;
								}

								this.field466 = var5 + var5 - 1 - this.field466;
								this.field453 = -this.field453;
								if (--this.numLoops == 0) {
									break label127;
								}
							}

							do {
								var9 = this.method908(var1, var9, var6, var3, var4.samples[this.end - 1]);
								if (this.field466 < var6) {
									return;
								}

								this.field466 = var6 + var6 - 1 - this.field466;
								this.field453 = -this.field453;
								if (--this.numLoops == 0) {
									break;
								}

								var9 = this.method909(var1, var9, var5, var3, var4.samples[this.start]);
								if (this.field466 >= var5) {
									return;
								}

								this.field466 = var5 + var5 - 1 - this.field466;
								this.field453 = -this.field453;
							} while(--this.numLoops != 0);
						}
					} else {
						int var10;
						if (this.field453 < 0) {
							while (true) {
								var9 = this.method909(var1, var9, var5, var3, var4.samples[this.end - 1]);
								if (this.field466 >= var5) {
									return;
								}

								var10 = (var6 - 1 - this.field466) / var8;
								if (var10 >= this.numLoops) {
									this.field466 += var8 * this.numLoops;
									this.numLoops = 0;
									break;
								}

								this.field466 += var8 * var10;
								this.numLoops -= var10;
							}
						} else {
							while (true) {
								var9 = this.method908(var1, var9, var6, var3, var4.samples[this.start]);
								if (this.field466 < var6) {
									return;
								}

								var10 = (this.field466 - var5) / var8;
								if (var10 >= this.numLoops) {
									this.field466 -= var8 * this.numLoops;
									this.numLoops = 0;
									break;
								}

								this.field466 -= var8 * var10;
								this.numLoops -= var10;
							}
						}
					}
				}

				if (this.field453 < 0) {
					this.method909(var1, var9, 0, var3, 0);
					if (this.field466 < 0) {
						this.field466 = -1;
						this.method917();
						this.remove();
					}
				} else {
					this.method908(var1, var9, var7, var3, 0);
					if (this.field466 >= var7) {
						this.field466 = var7;
						this.method917();
						this.remove();
					}
				}

			}
		}
	}

	@ObfuscatedName("q")
	@Export("setNumLoops")
	public synchronized void setNumLoops(int var1) {
		this.numLoops = var1;
	}

	@ObfuscatedName("i")
	@Export("skip")
	public synchronized void skip(int var1) {
		if (this.field454 > 0) {
			if (var1 >= this.field454) {
				if (this.field460 == Integer.MIN_VALUE) {
					this.field460 = 0;
					this.field452 = 0;
					this.field457 = 0;
					this.field463 = 0;
					this.remove();
					var1 = this.field454;
				}

				this.field454 = 0;
				this.method886();
			} else {
				this.field463 += this.field464 * var1;
				this.field457 += this.field465 * var1;
				this.field452 += this.field461 * var1;
				this.field454 -= var1;
			}
		}

		RawSound var2 = (RawSound)super.sound;
		int var3 = this.start << 8;
		int var4 = this.end << 8;
		int var5 = var2.samples.length << 8;
		int var6 = var4 - var3;
		if (var6 <= 0) {
			this.numLoops = 0;
		}

		if (this.field466 < 0) {
			if (this.field453 <= 0) {
				this.method917();
				this.remove();
				return;
			}

			this.field466 = 0;
		}

		if (this.field466 >= var5) {
			if (this.field453 >= 0) {
				this.method917();
				this.remove();
				return;
			}

			this.field466 = var5 - 1;
		}

		this.field466 += this.field453 * var1;
		if (this.numLoops < 0) {
			if (!this.field462) {
				if (this.field453 < 0) {
					if (this.field466 >= var3) {
						return;
					}

					this.field466 = var4 - 1 - (var4 - 1 - this.field466) % var6;
				} else {
					if (this.field466 < var4) {
						return;
					}

					this.field466 = var3 + (this.field466 - var3) % var6;
				}

			} else {
				if (this.field453 < 0) {
					if (this.field466 >= var3) {
						return;
					}

					this.field466 = var3 + var3 - 1 - this.field466;
					this.field453 = -this.field453;
				}

				while (this.field466 >= var4) {
					this.field466 = var4 + var4 - 1 - this.field466;
					this.field453 = -this.field453;
					if (this.field466 >= var3) {
						return;
					}

					this.field466 = var3 + var3 - 1 - this.field466;
					this.field453 = -this.field453;
				}

			}
		} else {
			if (this.numLoops > 0) {
				if (this.field462) {
					label129: {
						if (this.field453 < 0) {
							if (this.field466 >= var3) {
								return;
							}

							this.field466 = var3 + var3 - 1 - this.field466;
							this.field453 = -this.field453;
							if (--this.numLoops == 0) {
								break label129;
							}
						}

						do {
							if (this.field466 < var4) {
								return;
							}

							this.field466 = var4 + var4 - 1 - this.field466;
							this.field453 = -this.field453;
							if (--this.numLoops == 0) {
								break;
							}

							if (this.field466 >= var3) {
								return;
							}

							this.field466 = var3 + var3 - 1 - this.field466;
							this.field453 = -this.field453;
						} while(--this.numLoops != 0);
					}
				} else {
					label161: {
						int var7;
						if (this.field453 < 0) {
							if (this.field466 >= var3) {
								return;
							}

							var7 = (var4 - 1 - this.field466) / var6;
							if (var7 >= this.numLoops) {
								this.field466 += var6 * this.numLoops;
								this.numLoops = 0;
								break label161;
							}

							this.field466 += var6 * var7;
							this.numLoops -= var7;
						} else {
							if (this.field466 < var4) {
								return;
							}

							var7 = (this.field466 - var3) / var6;
							if (var7 >= this.numLoops) {
								this.field466 -= var6 * this.numLoops;
								this.numLoops = 0;
								break label161;
							}

							this.field466 -= var6 * var7;
							this.numLoops -= var7;
						}

						return;
					}
				}
			}

			if (this.field453 < 0) {
				if (this.field466 < 0) {
					this.field466 = -1;
					this.method917();
					this.remove();
				}
			} else if (this.field466 >= var5) {
				this.field466 = var5;
				this.method917();
				this.remove();
			}

		}
	}

	@ObfuscatedName("x")
	public synchronized void method888(int var1) {
		this.method890(var1 << 6, this.method892());
	}

	@ObfuscatedName("k")
	synchronized void method889(int var1) {
		this.method890(var1, this.method892());
	}

	@ObfuscatedName("r")
	synchronized void method890(int var1, int var2) {
		this.field460 = var1;
		this.field455 = var2;
		this.field454 = 0;
		this.method886();
	}

	@ObfuscatedName("p")
	public synchronized int method891() {
		return this.field460 == Integer.MIN_VALUE ? 0 : this.field460;
	}

	@ObfuscatedName("a")
	public synchronized int method892() {
		return this.field455 < 0 ? -1 : this.field455;
	}

	@ObfuscatedName("e")
	public synchronized void method893(int var1) {
		int var2 = ((RawSound)super.sound).samples.length << 8;
		if (var1 < -1) {
			var1 = -1;
		}

		if (var1 > var2) {
			var1 = var2;
		}

		this.field466 = var1;
	}

	@ObfuscatedName("d")
	@ObfuscatedSignature(
		descriptor = "(Z)V",
		garbageValue = "1"
	)
	public synchronized void method959() {
		this.field453 = (this.field453 ^ this.field453 >> 31) + (this.field453 >>> 31);
		this.field453 = -this.field453;
	}

	@ObfuscatedName("u")
	void method917() {
		if (this.field454 != 0) {
			if (this.field460 == Integer.MIN_VALUE) {
				this.field460 = 0;
			}

			this.field454 = 0;
			this.method886();
		}

	}

	@ObfuscatedName("m")
	public synchronized void method896(int var1, int var2) {
		this.method897(var1, var2, this.method892());
	}

	@ObfuscatedName("j")
	public synchronized void method897(int var1, int var2, int var3) {
		if (var1 == 0) {
			this.method890(var2, var3);
		} else {
			int var4 = method924(var2, var3);
			int var5 = method880(var2, var3);
			if (var4 == this.field457 && var5 == this.field452) {
				this.field454 = 0;
			} else {
				int var6 = var2 - this.field463;
				if (this.field463 - var2 > var6) {
					var6 = this.field463 - var2;
				}

				if (var4 - this.field457 > var6) {
					var6 = var4 - this.field457;
				}

				if (this.field457 - var4 > var6) {
					var6 = this.field457 - var4;
				}

				if (var5 - this.field452 > var6) {
					var6 = var5 - this.field452;
				}

				if (this.field452 - var5 > var6) {
					var6 = this.field452 - var5;
				}

				if (var1 > var6) {
					var1 = var6;
				}

				this.field454 = var1;
				this.field460 = var2;
				this.field455 = var3;
				this.field464 = (var2 - this.field463) / var1;
				this.field465 = (var4 - this.field457) / var1;
				this.field461 = (var5 - this.field452) / var1;
			}
		}
	}

	@ObfuscatedName("f")
	public synchronized void method898(int var1) {
		if (var1 == 0) {
			this.method889(0);
			this.remove();
		} else if (this.field457 == 0 && this.field452 == 0) {
			this.field454 = 0;
			this.field460 = 0;
			this.field463 = 0;
			this.remove();
		} else {
			int var2 = -this.field463;
			if (this.field463 > var2) {
				var2 = this.field463;
			}

			if (-this.field457 > var2) {
				var2 = -this.field457;
			}

			if (this.field457 > var2) {
				var2 = this.field457;
			}

			if (-this.field452 > var2) {
				var2 = -this.field452;
			}

			if (this.field452 > var2) {
				var2 = this.field452;
			}

			if (var1 > var2) {
				var1 = var2;
			}

			this.field454 = var1;
			this.field460 = Integer.MIN_VALUE;
			this.field464 = -this.field463 / var1;
			this.field465 = -this.field457 / var1;
			this.field461 = -this.field452 / var1;
		}
	}

	@ObfuscatedName("aw")
	public synchronized void method899(int var1) {
		if (this.field453 < 0) {
			this.field453 = -var1;
		} else {
			this.field453 = var1;
		}

	}

	@ObfuscatedName("ap")
	public synchronized int method935() {
		return this.field453 < 0 ? -this.field453 : this.field453;
	}

	@ObfuscatedName("am")
	public boolean method1004() {
		return this.field466 < 0 || this.field466 >= ((RawSound)super.sound).samples.length << 8;
	}

	@ObfuscatedName("ax")
	int vmethod1044() {
		int var1 = this.field463 * 3 >> 6;
		var1 = (var1 ^ var1 >> 31) + (var1 >>> 31);
		if (this.numLoops == 0) {
			var1 -= var1 * this.field466 / (((RawSound)super.sound).samples.length << 8);
		} else if (this.numLoops >= 0) {
			var1 -= var1 * this.start / ((RawSound)super.sound).samples.length;
		}

		return var1 > 255 ? 255 : var1;
	}

	@ObfuscatedName("ak")
	public boolean method902() {
		return this.field454 != 0;
	}

	@ObfuscatedName("ay")
	int method908(int[] var1, int var2, int var3, int var4, int var5) {
		while (true) {
			if (this.field454 > 0) {
				int var6 = var2 + this.field454;
				if (var6 > var4) {
					var6 = var4;
				}

				this.field454 += var2;
				if (this.field453 == 256 && (this.field466 & 255) == 0) {
					if (ApproximateRouteStrategy.PcmPlayer_stereo) {
						var2 = method920(0, ((RawSound)super.sound).samples, var1, this.field466, var2, this.field457, this.field452, this.field465, this.field461, 0, var6, var3, this);
					} else {
						var2 = method919(((RawSound)super.sound).samples, var1, this.field466, var2, this.field463, this.field464, 0, var6, var3, this);
					}
				} else if (ApproximateRouteStrategy.PcmPlayer_stereo) {
					var2 = method936(0, 0, ((RawSound)super.sound).samples, var1, this.field466, var2, this.field457, this.field452, this.field465, this.field461, 0, var6, var3, this, this.field453, var5);
				} else {
					var2 = method987(0, 0, ((RawSound)super.sound).samples, var1, this.field466, var2, this.field463, this.field464, 0, var6, var3, this, this.field453, var5);
				}

				this.field454 -= var2;
				if (this.field454 != 0) {
					return var2;
				}

				if (!this.method910()) {
					continue;
				}

				return var4;
			}

			if (this.field453 == 256 && (this.field466 & 255) == 0) {
				if (ApproximateRouteStrategy.PcmPlayer_stereo) {
					return method912(0, ((RawSound)super.sound).samples, var1, this.field466, var2, this.field457, this.field452, 0, var4, var3, this);
				}

				return method911(((RawSound)super.sound).samples, var1, this.field466, var2, this.field463, 0, var4, var3, this);
			}

			if (ApproximateRouteStrategy.PcmPlayer_stereo) {
				return method916(0, 0, ((RawSound)super.sound).samples, var1, this.field466, var2, this.field457, this.field452, 0, var4, var3, this, this.field453, var5);
			}

			return method915(0, 0, ((RawSound)super.sound).samples, var1, this.field466, var2, this.field463, 0, var4, var3, this, this.field453, var5);
		}
	}

	@ObfuscatedName("ai")
	int method909(int[] var1, int var2, int var3, int var4, int var5) {
		while (true) {
			if (this.field454 > 0) {
				int var6 = var2 + this.field454;
				if (var6 > var4) {
					var6 = var4;
				}

				this.field454 += var2;
				if (this.field453 == -256 && (this.field466 & 255) == 0) {
					if (ApproximateRouteStrategy.PcmPlayer_stereo) {
						var2 = method914(0, ((RawSound)super.sound).samples, var1, this.field466, var2, this.field457, this.field452, this.field465, this.field461, 0, var6, var3, this);
					} else {
						var2 = method921(((RawSound)super.sound).samples, var1, this.field466, var2, this.field463, this.field464, 0, var6, var3, this);
					}
				} else if (ApproximateRouteStrategy.PcmPlayer_stereo) {
					var2 = method926(0, 0, ((RawSound)super.sound).samples, var1, this.field466, var2, this.field457, this.field452, this.field465, this.field461, 0, var6, var3, this, this.field453, var5);
				} else {
					var2 = method925(0, 0, ((RawSound)super.sound).samples, var1, this.field466, var2, this.field463, this.field464, 0, var6, var3, this, this.field453, var5);
				}

				this.field454 -= var2;
				if (this.field454 != 0) {
					return var2;
				}

				if (!this.method910()) {
					continue;
				}

				return var4;
			}

			if (this.field453 == -256 && (this.field466 & 255) == 0) {
				if (ApproximateRouteStrategy.PcmPlayer_stereo) {
					return method1013(0, ((RawSound)super.sound).samples, var1, this.field466, var2, this.field457, this.field452, 0, var4, var3, this);
				}

				return method913(((RawSound)super.sound).samples, var1, this.field466, var2, this.field463, 0, var4, var3, this);
			}

			if (ApproximateRouteStrategy.PcmPlayer_stereo) {
				return method962(0, 0, ((RawSound)super.sound).samples, var1, this.field466, var2, this.field457, this.field452, 0, var4, var3, this, this.field453, var5);
			}

			return method968(0, 0, ((RawSound)super.sound).samples, var1, this.field466, var2, this.field463, 0, var4, var3, this, this.field453, var5);
		}
	}

	@ObfuscatedName("aa")
	boolean method910() {
		int var1 = this.field460;
		int var2;
		int var3;
		if (var1 == Integer.MIN_VALUE) {
			var3 = 0;
			var2 = 0;
			var1 = 0;
		} else {
			var2 = method924(var1, this.field455);
			var3 = method880(var1, this.field455);
		}

		if (var1 == this.field463 && var2 == this.field457 && var3 == this.field452) {
			if (this.field460 == Integer.MIN_VALUE) {
				this.field460 = 0;
				this.field452 = 0;
				this.field457 = 0;
				this.field463 = 0;
				this.remove();
				return true;
			} else {
				this.method886();
				return false;
			}
		} else {
			if (this.field463 < var1) {
				this.field464 = 1;
				this.field454 = var1 - this.field463;
			} else if (this.field463 > var1) {
				this.field464 = -1;
				this.field454 = this.field463 - var1;
			} else {
				this.field464 = 0;
			}

			if (this.field457 < var2) {
				this.field465 = 1;
				if (this.field454 == 0 || this.field454 > var2 - this.field457) {
					this.field454 = var2 - this.field457;
				}
			} else if (this.field457 > var2) {
				this.field465 = -1;
				if (this.field454 == 0 || this.field454 > this.field457 - var2) {
					this.field454 = this.field457 - var2;
				}
			} else {
				this.field465 = 0;
			}

			if (this.field452 < var3) {
				this.field461 = 1;
				if (this.field454 == 0 || this.field454 > var3 - this.field452) {
					this.field454 = var3 - this.field452;
				}
			} else if (this.field452 > var3) {
				this.field461 = -1;
				if (this.field454 == 0 || this.field454 > this.field452 - var3) {
					this.field454 = this.field452 - var3;
				}
			} else {
				this.field461 = 0;
			}

			return false;
		}
	}

	@ObfuscatedName("h")
	static int method924(int var0, int var1) {
		return var1 < 0 ? var0 : (int)((double)var0 * Math.sqrt((double)(16384 - var1) * 1.220703125E-4D) + 0.5D);
	}

	@ObfuscatedName("c")
	static int method880(int var0, int var1) {
		return var1 < 0 ? -var0 : (int)((double)var0 * Math.sqrt((double)var1 * 1.220703125E-4D) + 0.5D);
	}

	@ObfuscatedName("o")
	@ObfuscatedSignature(
		descriptor = "(Laf;II)Lbk;"
	)
	@Export("createRawPcmStream")
	public static RawPcmStream createRawPcmStream(RawSound var0, int var1, int var2) {
		return var0.samples != null && var0.samples.length != 0 ? new RawPcmStream(var0, (int)((long)var0.sampleRate * 256L * (long)var1 / (long)(PcmPlayer.field401 * 100)), var2 << 6) : null;
	}

	@ObfuscatedName("g")
	@ObfuscatedSignature(
		descriptor = "(Laf;III)Lbk;"
	)
	public static RawPcmStream method885(RawSound var0, int var1, int var2, int var3) {
		return var0.samples != null && var0.samples.length != 0 ? new RawPcmStream(var0, var1, var2, var3) : null;
	}

	@ObfuscatedName("au")
	@ObfuscatedSignature(
		descriptor = "([B[IIIIIIILbk;)I"
	)
	static int method911(byte[] var0, int[] var1, int var2, int var3, int var4, int var5, int var6, int var7, RawPcmStream var8) {
		var2 >>= 8;
		var7 >>= 8;
		var4 <<= 2;
		if ((var5 = var3 + var7 - var2) > var6) {
			var5 = var6;
		}

		int var10001;
		for (var5 -= 3; var3 < var5; var1[var10001] += var0[var2++] * var4) {
			var10001 = var3++;
			var1[var10001] += var0[var2++] * var4;
			var10001 = var3++;
			var1[var10001] += var0[var2++] * var4;
			var10001 = var3++;
			var1[var10001] += var0[var2++] * var4;
			var10001 = var3++;
		}

		for (var5 += 3; var3 < var5; var1[var10001] += var0[var2++] * var4) {
			var10001 = var3++;
		}

		var8.field466 = var2 << 8;
		return var3;
	}

	@ObfuscatedName("as")
	@ObfuscatedSignature(
		descriptor = "(I[B[IIIIIIIILbk;)I"
	)
	static int method912(int var0, byte[] var1, int[] var2, int var3, int var4, int var5, int var6, int var7, int var8, int var9, RawPcmStream var10) {
		var3 >>= 8;
		var9 >>= 8;
		var5 <<= 2;
		var6 <<= 2;
		if ((var7 = var4 + var9 - var3) > var8) {
			var7 = var8;
		}

		var4 <<= 1;
		var7 <<= 1;

		int var10001;
		byte var11;
		for (var7 -= 6; var4 < var7; var2[var10001] += var11 * var6) {
			var11 = var1[var3++];
			var10001 = var4++;
			var2[var10001] += var11 * var5;
			var10001 = var4++;
			var2[var10001] += var11 * var6;
			var11 = var1[var3++];
			var10001 = var4++;
			var2[var10001] += var11 * var5;
			var10001 = var4++;
			var2[var10001] += var11 * var6;
			var11 = var1[var3++];
			var10001 = var4++;
			var2[var10001] += var11 * var5;
			var10001 = var4++;
			var2[var10001] += var11 * var6;
			var11 = var1[var3++];
			var10001 = var4++;
			var2[var10001] += var11 * var5;
			var10001 = var4++;
		}

		for (var7 += 6; var4 < var7; var2[var10001] += var11 * var6) {
			var11 = var1[var3++];
			var10001 = var4++;
			var2[var10001] += var11 * var5;
			var10001 = var4++;
		}

		var10.field466 = var3 << 8;
		return var4 >> 1;
	}

	@ObfuscatedName("ad")
	@ObfuscatedSignature(
		descriptor = "([B[IIIIIIILbk;)I"
	)
	static int method913(byte[] var0, int[] var1, int var2, int var3, int var4, int var5, int var6, int var7, RawPcmStream var8) {
		var2 >>= 8;
		var7 >>= 8;
		var4 <<= 2;
		if ((var5 = var3 + var2 - (var7 - 1)) > var6) {
			var5 = var6;
		}

		int var10001;
		for (var5 -= 3; var3 < var5; var1[var10001] += var0[var2--] * var4) {
			var10001 = var3++;
			var1[var10001] += var0[var2--] * var4;
			var10001 = var3++;
			var1[var10001] += var0[var2--] * var4;
			var10001 = var3++;
			var1[var10001] += var0[var2--] * var4;
			var10001 = var3++;
		}

		for (var5 += 3; var3 < var5; var1[var10001] += var0[var2--] * var4) {
			var10001 = var3++;
		}

		var8.field466 = var2 << 8;
		return var3;
	}

	@ObfuscatedName("ac")
	@ObfuscatedSignature(
		descriptor = "(I[B[IIIIIIIILbk;)I"
	)
	static int method1013(int var0, byte[] var1, int[] var2, int var3, int var4, int var5, int var6, int var7, int var8, int var9, RawPcmStream var10) {
		var3 >>= 8;
		var9 >>= 8;
		var5 <<= 2;
		var6 <<= 2;
		if ((var7 = var3 + var4 - (var9 - 1)) > var8) {
			var7 = var8;
		}

		var4 <<= 1;
		var7 <<= 1;

		int var10001;
		byte var11;
		for (var7 -= 6; var4 < var7; var2[var10001] += var11 * var6) {
			var11 = var1[var3--];
			var10001 = var4++;
			var2[var10001] += var11 * var5;
			var10001 = var4++;
			var2[var10001] += var11 * var6;
			var11 = var1[var3--];
			var10001 = var4++;
			var2[var10001] += var11 * var5;
			var10001 = var4++;
			var2[var10001] += var11 * var6;
			var11 = var1[var3--];
			var10001 = var4++;
			var2[var10001] += var11 * var5;
			var10001 = var4++;
			var2[var10001] += var11 * var6;
			var11 = var1[var3--];
			var10001 = var4++;
			var2[var10001] += var11 * var5;
			var10001 = var4++;
		}

		for (var7 += 6; var4 < var7; var2[var10001] += var11 * var6) {
			var11 = var1[var3--];
			var10001 = var4++;
			var2[var10001] += var11 * var5;
			var10001 = var4++;
		}

		var10.field466 = var3 << 8;
		return var4 >> 1;
	}

	@ObfuscatedName("az")
	@ObfuscatedSignature(
		descriptor = "(II[B[IIIIIIILbk;II)I"
	)
	static int method915(int var0, int var1, byte[] var2, int[] var3, int var4, int var5, int var6, int var7, int var8, int var9, RawPcmStream var10, int var11, int var12) {
		if (var11 == 0 || (var7 = var5 + (var11 + (var9 - var4) - 257) / var11) > var8) {
			var7 = var8;
		}

		byte var13;
		int var10001;
		while (var5 < var7) {
			var1 = var4 >> 8;
			var13 = var2[var1];
			var10001 = var5++;
			var3[var10001] += ((var13 << 8) + (var2[var1 + 1] - var13) * (var4 & 255)) * var6 >> 6;
			var4 += var11;
		}

		if (var11 == 0 || (var7 = var5 + (var11 + (var9 - var4) - 1) / var11) > var8) {
			var7 = var8;
		}

		for (var1 = var12; var5 < var7; var4 += var11) {
			var13 = var2[var4 >> 8];
			var10001 = var5++;
			var3[var10001] += ((var13 << 8) + (var1 - var13) * (var4 & 255)) * var6 >> 6;
		}

		var10.field466 = var4;
		return var5;
	}

	@ObfuscatedName("aq")
	@ObfuscatedSignature(
		descriptor = "(II[B[IIIIIIIILbk;II)I"
	)
	static int method916(int var0, int var1, byte[] var2, int[] var3, int var4, int var5, int var6, int var7, int var8, int var9, int var10, RawPcmStream var11, int var12, int var13) {
		if (var12 == 0 || (var8 = var5 + (var10 - var4 + var12 - 257) / var12) > var9) {
			var8 = var9;
		}

		var5 <<= 1;

		byte var14;
		int var10001;
		for (var8 <<= 1; var5 < var8; var4 += var12) {
			var1 = var4 >> 8;
			var14 = var2[var1];
			var0 = (var14 << 8) + (var4 & 255) * (var2[var1 + 1] - var14);
			var10001 = var5++;
			var3[var10001] += var0 * var6 >> 6;
			var10001 = var5++;
			var3[var10001] += var0 * var7 >> 6;
		}

		if (var12 == 0 || (var8 = (var5 >> 1) + (var10 - var4 + var12 - 1) / var12) > var9) {
			var8 = var9;
		}

		var8 <<= 1;

		for (var1 = var13; var5 < var8; var4 += var12) {
			var14 = var2[var4 >> 8];
			var0 = (var14 << 8) + (var1 - var14) * (var4 & 255);
			var10001 = var5++;
			var3[var10001] += var0 * var6 >> 6;
			var10001 = var5++;
			var3[var10001] += var0 * var7 >> 6;
		}

		var11.field466 = var4;
		return var5 >> 1;
	}

	@ObfuscatedName("ab")
	@ObfuscatedSignature(
		descriptor = "(II[B[IIIIIIILbk;II)I"
	)
	static int method968(int var0, int var1, byte[] var2, int[] var3, int var4, int var5, int var6, int var7, int var8, int var9, RawPcmStream var10, int var11, int var12) {
		if (var11 == 0 || (var7 = var5 + (var11 + (var9 + 256 - var4)) / var11) > var8) {
			var7 = var8;
		}

		int var10001;
		while (var5 < var7) {
			var1 = var4 >> 8;
			byte var13 = var2[var1 - 1];
			var10001 = var5++;
			var3[var10001] += ((var13 << 8) + (var2[var1] - var13) * (var4 & 255)) * var6 >> 6;
			var4 += var11;
		}

		if (var11 == 0 || (var7 = var5 + (var11 + (var9 - var4)) / var11) > var8) {
			var7 = var8;
		}

		var0 = var12;

		for (var1 = var11; var5 < var7; var4 += var1) {
			var10001 = var5++;
			var3[var10001] += ((var0 << 8) + (var2[var4 >> 8] - var0) * (var4 & 255)) * var6 >> 6;
		}

		var10.field466 = var4;
		return var5;
	}

	@ObfuscatedName("ar")
	@ObfuscatedSignature(
		descriptor = "(II[B[IIIIIIIILbk;II)I"
	)
	static int method962(int var0, int var1, byte[] var2, int[] var3, int var4, int var5, int var6, int var7, int var8, int var9, int var10, RawPcmStream var11, int var12, int var13) {
		if (var12 == 0 || (var8 = var5 + (var10 + 256 - var4 + var12) / var12) > var9) {
			var8 = var9;
		}

		var5 <<= 1;

		int var10001;
		for (var8 <<= 1; var5 < var8; var4 += var12) {
			var1 = var4 >> 8;
			byte var14 = var2[var1 - 1];
			var0 = (var2[var1] - var14) * (var4 & 255) + (var14 << 8);
			var10001 = var5++;
			var3[var10001] += var0 * var6 >> 6;
			var10001 = var5++;
			var3[var10001] += var0 * var7 >> 6;
		}

		if (var12 == 0 || (var8 = (var5 >> 1) + (var10 - var4 + var12) / var12) > var9) {
			var8 = var9;
		}

		var8 <<= 1;

		for (var1 = var13; var5 < var8; var4 += var12) {
			var0 = (var1 << 8) + (var4 & 255) * (var2[var4 >> 8] - var1);
			var10001 = var5++;
			var3[var10001] += var0 * var6 >> 6;
			var10001 = var5++;
			var3[var10001] += var0 * var7 >> 6;
		}

		var11.field466 = var4;
		return var5 >> 1;
	}

	@ObfuscatedName("ah")
	@ObfuscatedSignature(
		descriptor = "([B[IIIIIIIILbk;)I"
	)
	static int method919(byte[] var0, int[] var1, int var2, int var3, int var4, int var5, int var6, int var7, int var8, RawPcmStream var9) {
		var2 >>= 8;
		var8 >>= 8;
		var4 <<= 2;
		var5 <<= 2;
		if ((var6 = var3 + var8 - var2) > var7) {
			var6 = var7;
		}

		var9.field457 += var9.field465 * (var6 - var3);
		var9.field452 += var9.field461 * (var6 - var3);

		int var10001;
		for (var6 -= 3; var3 < var6; var4 += var5) {
			var10001 = var3++;
			var1[var10001] += var0[var2++] * var4;
			var4 += var5;
			var10001 = var3++;
			var1[var10001] += var0[var2++] * var4;
			var4 += var5;
			var10001 = var3++;
			var1[var10001] += var0[var2++] * var4;
			var4 += var5;
			var10001 = var3++;
			var1[var10001] += var0[var2++] * var4;
		}

		for (var6 += 3; var3 < var6; var4 += var5) {
			var10001 = var3++;
			var1[var10001] += var0[var2++] * var4;
		}

		var9.field463 = var4 >> 2;
		var9.field466 = var2 << 8;
		return var3;
	}

	@ObfuscatedName("af")
	@ObfuscatedSignature(
		descriptor = "(I[B[IIIIIIIIIILbk;)I"
	)
	static int method920(int var0, byte[] var1, int[] var2, int var3, int var4, int var5, int var6, int var7, int var8, int var9, int var10, int var11, RawPcmStream var12) {
		var3 >>= 8;
		var11 >>= 8;
		var5 <<= 2;
		var6 <<= 2;
		var7 <<= 2;
		var8 <<= 2;
		if ((var9 = var11 + var4 - var3) > var10) {
			var9 = var10;
		}

		var12.field463 += var12.field464 * (var9 - var4);
		var4 <<= 1;
		var9 <<= 1;

		byte var13;
		int var10001;
		for (var9 -= 6; var4 < var9; var6 += var8) {
			var13 = var1[var3++];
			var10001 = var4++;
			var2[var10001] += var13 * var5;
			var5 += var7;
			var10001 = var4++;
			var2[var10001] += var13 * var6;
			var6 += var8;
			var13 = var1[var3++];
			var10001 = var4++;
			var2[var10001] += var13 * var5;
			var5 += var7;
			var10001 = var4++;
			var2[var10001] += var13 * var6;
			var6 += var8;
			var13 = var1[var3++];
			var10001 = var4++;
			var2[var10001] += var13 * var5;
			var5 += var7;
			var10001 = var4++;
			var2[var10001] += var13 * var6;
			var6 += var8;
			var13 = var1[var3++];
			var10001 = var4++;
			var2[var10001] += var13 * var5;
			var5 += var7;
			var10001 = var4++;
			var2[var10001] += var13 * var6;
		}

		for (var9 += 6; var4 < var9; var6 += var8) {
			var13 = var1[var3++];
			var10001 = var4++;
			var2[var10001] += var13 * var5;
			var5 += var7;
			var10001 = var4++;
			var2[var10001] += var13 * var6;
		}

		var12.field457 = var5 >> 2;
		var12.field452 = var6 >> 2;
		var12.field466 = var3 << 8;
		return var4 >> 1;
	}

	@ObfuscatedName("an")
	@ObfuscatedSignature(
		descriptor = "([B[IIIIIIIILbk;)I"
	)
	static int method921(byte[] var0, int[] var1, int var2, int var3, int var4, int var5, int var6, int var7, int var8, RawPcmStream var9) {
		var2 >>= 8;
		var8 >>= 8;
		var4 <<= 2;
		var5 <<= 2;
		if ((var6 = var3 + var2 - (var8 - 1)) > var7) {
			var6 = var7;
		}

		var9.field457 += var9.field465 * (var6 - var3);
		var9.field452 += var9.field461 * (var6 - var3);

		int var10001;
		for (var6 -= 3; var3 < var6; var4 += var5) {
			var10001 = var3++;
			var1[var10001] += var0[var2--] * var4;
			var4 += var5;
			var10001 = var3++;
			var1[var10001] += var0[var2--] * var4;
			var4 += var5;
			var10001 = var3++;
			var1[var10001] += var0[var2--] * var4;
			var4 += var5;
			var10001 = var3++;
			var1[var10001] += var0[var2--] * var4;
		}

		for (var6 += 3; var3 < var6; var4 += var5) {
			var10001 = var3++;
			var1[var10001] += var0[var2--] * var4;
		}

		var9.field463 = var4 >> 2;
		var9.field466 = var2 << 8;
		return var3;
	}

	@ObfuscatedName("bd")
	@ObfuscatedSignature(
		descriptor = "(I[B[IIIIIIIIIILbk;)I"
	)
	static int method914(int var0, byte[] var1, int[] var2, int var3, int var4, int var5, int var6, int var7, int var8, int var9, int var10, int var11, RawPcmStream var12) {
		var3 >>= 8;
		var11 >>= 8;
		var5 <<= 2;
		var6 <<= 2;
		var7 <<= 2;
		var8 <<= 2;
		if ((var9 = var3 + var4 - (var11 - 1)) > var10) {
			var9 = var10;
		}

		var12.field463 += var12.field464 * (var9 - var4);
		var4 <<= 1;
		var9 <<= 1;

		byte var13;
		int var10001;
		for (var9 -= 6; var4 < var9; var6 += var8) {
			var13 = var1[var3--];
			var10001 = var4++;
			var2[var10001] += var13 * var5;
			var5 += var7;
			var10001 = var4++;
			var2[var10001] += var13 * var6;
			var6 += var8;
			var13 = var1[var3--];
			var10001 = var4++;
			var2[var10001] += var13 * var5;
			var5 += var7;
			var10001 = var4++;
			var2[var10001] += var13 * var6;
			var6 += var8;
			var13 = var1[var3--];
			var10001 = var4++;
			var2[var10001] += var13 * var5;
			var5 += var7;
			var10001 = var4++;
			var2[var10001] += var13 * var6;
			var6 += var8;
			var13 = var1[var3--];
			var10001 = var4++;
			var2[var10001] += var13 * var5;
			var5 += var7;
			var10001 = var4++;
			var2[var10001] += var13 * var6;
		}

		for (var9 += 6; var4 < var9; var6 += var8) {
			var13 = var1[var3--];
			var10001 = var4++;
			var2[var10001] += var13 * var5;
			var5 += var7;
			var10001 = var4++;
			var2[var10001] += var13 * var6;
		}

		var12.field457 = var5 >> 2;
		var12.field452 = var6 >> 2;
		var12.field466 = var3 << 8;
		return var4 >> 1;
	}

	@ObfuscatedName("bw")
	@ObfuscatedSignature(
		descriptor = "(II[B[IIIIIIIILbk;II)I"
	)
	static int method987(int var0, int var1, byte[] var2, int[] var3, int var4, int var5, int var6, int var7, int var8, int var9, int var10, RawPcmStream var11, int var12, int var13) {
		var11.field457 -= var11.field465 * var5;
		var11.field452 -= var11.field461 * var5;
		if (var12 == 0 || (var8 = var5 + (var10 - var4 + var12 - 257) / var12) > var9) {
			var8 = var9;
		}

		byte var14;
		int var10001;
		while (var5 < var8) {
			var1 = var4 >> 8;
			var14 = var2[var1];
			var10001 = var5++;
			var3[var10001] += ((var14 << 8) + (var2[var1 + 1] - var14) * (var4 & 255)) * var6 >> 6;
			var6 += var7;
			var4 += var12;
		}

		if (var12 == 0 || (var8 = var5 + (var10 - var4 + var12 - 1) / var12) > var9) {
			var8 = var9;
		}

		for (var1 = var13; var5 < var8; var4 += var12) {
			var14 = var2[var4 >> 8];
			var10001 = var5++;
			var3[var10001] += ((var14 << 8) + (var1 - var14) * (var4 & 255)) * var6 >> 6;
			var6 += var7;
		}

		var11.field457 += var11.field465 * var5;
		var11.field452 += var11.field461 * var5;
		var11.field463 = var6;
		var11.field466 = var4;
		return var5;
	}

	@ObfuscatedName("bf")
	@ObfuscatedSignature(
		descriptor = "(II[B[IIIIIIIIIILbk;II)I"
	)
	static int method936(int var0, int var1, byte[] var2, int[] var3, int var4, int var5, int var6, int var7, int var8, int var9, int var10, int var11, int var12, RawPcmStream var13, int var14, int var15) {
		var13.field463 -= var5 * var13.field464;
		if (var14 == 0 || (var10 = var5 + (var12 - var4 + var14 - 257) / var14) > var11) {
			var10 = var11;
		}

		var5 <<= 1;

		byte var16;
		int var10001;
		for (var10 <<= 1; var5 < var10; var4 += var14) {
			var1 = var4 >> 8;
			var16 = var2[var1];
			var0 = (var16 << 8) + (var4 & 255) * (var2[var1 + 1] - var16);
			var10001 = var5++;
			var3[var10001] += var0 * var6 >> 6;
			var6 += var8;
			var10001 = var5++;
			var3[var10001] += var0 * var7 >> 6;
			var7 += var9;
		}

		if (var14 == 0 || (var10 = (var5 >> 1) + (var12 - var4 + var14 - 1) / var14) > var11) {
			var10 = var11;
		}

		var10 <<= 1;

		for (var1 = var15; var5 < var10; var4 += var14) {
			var16 = var2[var4 >> 8];
			var0 = (var16 << 8) + (var1 - var16) * (var4 & 255);
			var10001 = var5++;
			var3[var10001] += var0 * var6 >> 6;
			var6 += var8;
			var10001 = var5++;
			var3[var10001] += var0 * var7 >> 6;
			var7 += var9;
		}

		var5 >>= 1;
		var13.field463 += var13.field464 * var5;
		var13.field457 = var6;
		var13.field452 = var7;
		var13.field466 = var4;
		return var5;
	}

	@ObfuscatedName("bu")
	@ObfuscatedSignature(
		descriptor = "(II[B[IIIIIIIILbk;II)I"
	)
	static int method925(int var0, int var1, byte[] var2, int[] var3, int var4, int var5, int var6, int var7, int var8, int var9, int var10, RawPcmStream var11, int var12, int var13) {
		var11.field457 -= var11.field465 * var5;
		var11.field452 -= var11.field461 * var5;
		if (var12 == 0 || (var8 = var5 + (var10 + 256 - var4 + var12) / var12) > var9) {
			var8 = var9;
		}

		int var10001;
		while (var5 < var8) {
			var1 = var4 >> 8;
			byte var14 = var2[var1 - 1];
			var10001 = var5++;
			var3[var10001] += ((var14 << 8) + (var2[var1] - var14) * (var4 & 255)) * var6 >> 6;
			var6 += var7;
			var4 += var12;
		}

		if (var12 == 0 || (var8 = var5 + (var10 - var4 + var12) / var12) > var9) {
			var8 = var9;
		}

		var0 = var13;

		for (var1 = var12; var5 < var8; var4 += var1) {
			var10001 = var5++;
			var3[var10001] += ((var0 << 8) + (var2[var4 >> 8] - var0) * (var4 & 255)) * var6 >> 6;
			var6 += var7;
		}

		var11.field457 += var11.field465 * var5;
		var11.field452 += var11.field461 * var5;
		var11.field463 = var6;
		var11.field466 = var4;
		return var5;
	}

	@ObfuscatedName("bb")
	@ObfuscatedSignature(
		descriptor = "(II[B[IIIIIIIIIILbk;II)I"
	)
	static int method926(int var0, int var1, byte[] var2, int[] var3, int var4, int var5, int var6, int var7, int var8, int var9, int var10, int var11, int var12, RawPcmStream var13, int var14, int var15) {
		var13.field463 -= var5 * var13.field464;
		if (var14 == 0 || (var10 = var5 + (var12 + 256 - var4 + var14) / var14) > var11) {
			var10 = var11;
		}

		var5 <<= 1;

		int var10001;
		for (var10 <<= 1; var5 < var10; var4 += var14) {
			var1 = var4 >> 8;
			byte var16 = var2[var1 - 1];
			var0 = (var2[var1] - var16) * (var4 & 255) + (var16 << 8);
			var10001 = var5++;
			var3[var10001] += var0 * var6 >> 6;
			var6 += var8;
			var10001 = var5++;
			var3[var10001] += var0 * var7 >> 6;
			var7 += var9;
		}

		if (var14 == 0 || (var10 = (var5 >> 1) + (var12 - var4 + var14) / var14) > var11) {
			var10 = var11;
		}

		var10 <<= 1;

		for (var1 = var15; var5 < var10; var4 += var14) {
			var0 = (var1 << 8) + (var4 & 255) * (var2[var4 >> 8] - var1);
			var10001 = var5++;
			var3[var10001] += var0 * var6 >> 6;
			var6 += var8;
			var10001 = var5++;
			var3[var10001] += var0 * var7 >> 6;
			var7 += var9;
		}

		var5 >>= 1;
		var13.field463 += var13.field464 * var5;
		var13.field457 = var6;
		var13.field452 = var7;
		var13.field466 = var4;
		return var5;
	}
}
