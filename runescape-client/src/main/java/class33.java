					...
					if (var0 == ScriptOpcodes.SETFOLLOWEROPSLOWPRIORITY) { // L: 4018
						...
						if (var0 == ScriptOpcodes.NC_PARAM) { // L: 4022
							...
							if (var5.isString()) { // L: 4053
								Interpreter.Interpreter_stringStack[++class9.Interpreter_stringStackSize - 1] = FileSystem.ItemComposition_get(var3).getStringParam(var4, var5.defaultStr); // L: 4054
							} else {
								Interpreter.Interpreter_intStack[++class12.Interpreter_intStackSize - 1] = FileSystem.ItemComposition_get(var3).getIntParam(var4, var5.defaultInt); // L: 4057
							}
