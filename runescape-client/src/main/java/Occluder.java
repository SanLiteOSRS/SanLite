	...
	public static final PcmPlayer method3401(TaskHandler var0, int var1, int var2) {
		if (PcmPlayer.field1421 == 0) {
			...
			try {
				...
				if (PcmPlayer.field1423 > 0 && FillMode.soundSystem == null) {
					FillMode.soundSystem = new SoundSystem();
					PcmPlayer.soundSystemExecutor = Executors.newScheduledThreadPool(1);
					PcmPlayer.soundSystemExecutor.scheduleAtFixedRate(FillMode.soundSystem, 0L, 10L, TimeUnit.MILLISECONDS);
				}

				if (FillMode.soundSystem != null) {
					if (FillMode.soundSystem.players[var1] != null) {
						throw new IllegalArgumentException();
					}

					FillMode.soundSystem.players[var1] = var3;
				}
