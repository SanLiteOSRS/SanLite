...
public class PcmStreamMixer extends PcmStream {
	...
	public PcmStreamMixer() {
		...
		this.field275 = new NodeDeque(); // L: 10
		this.field274 = 0; // L: 11
		this.field277 = -1; // L: 12
	}
	...
	void method725() {
		if (this.field274 > 0) { // L: 23
			for (PcmStreamMixerListener var1 = (PcmStreamMixerListener)this.field275.last(); var1 != null; var1 = (PcmStreamMixerListener)this.field275.previous()) { // L: 24
				var1.field429 -= this.field274; // L: 25
			}

			this.field277 -= this.field274; // L: 27
	...
	void method738(Node var1, PcmStreamMixerListener var2) {
		while (this.field275.sentinel != var1 && ((PcmStreamMixerListener)var1).field429 <= var2.field429) { // L: 33
			var1 = var1.previous;
		}

		NodeDeque.NodeDeque_addBefore(var2, var1); // L: 34
		this.field277 = ((PcmStreamMixerListener)this.field275.sentinel.previous).field429; // L: 35
	} // L: 36
	...
	void method715(PcmStreamMixerListener var1) {
		var1.remove(); // L: 39
		var1.remove2(); // L: 40
		Node var2 = this.field275.sentinel.previous; // L: 41
		if (var2 == this.field275.sentinel) {
			this.field277 = -1; // L: 42
		} else {
			this.field277 = ((PcmStreamMixerListener)var2).field429; // L: 43
		}
	...
	public final synchronized void skip(int var1) {
		do {
			if (this.field277 < 0) { // L: 99
				this.skipSubStreams(var1); // L: 100
				return; // L: 101
			}

			if (this.field274 + var1 < this.field277) { // L: 103
				this.field274 += var1; // L: 104
				this.skipSubStreams(var1); // L: 105
				return; // L: 106
			}

			int var2 = this.field277 - this.field274; // L: 108
			this.skipSubStreams(var2); // L: 109
			var1 -= var2; // L: 110
			this.field274 += var2; // L: 111
			this.method725(); // L: 112
			PcmStreamMixerListener var3 = (PcmStreamMixerListener)this.field275.last(); // L: 113
			synchronized(var3) { // L: 114
				int var5 = var3.update(); // L: 115
				if (var5 < 0) { // L: 116
					var3.field429 = 0; // L: 117
					this.method715(var3); // L: 118
				} else {
					var3.field429 = var5; // L: 121
					this.method738(var3.previous, var3); // L: 122
				}
			}
		} while(var1 != 0); // L: 125
