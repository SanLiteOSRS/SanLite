...
public class VorbisCodebook {
	...
	VorbisCodebook() {
		class60 var1 = VorbisSample.field374; // L: 22
		var1.method1183(24); // L: 23
		this.field361 = var1.method1183(16); // L: 24
		this.entries = var1.method1183(24); // L: 25
		this.lengthMap = new int[this.entries]; // L: 26
		boolean var2 = var1.method1179() != 0; // L: 27
		int var3;
		int var4;
		int var6;
		if (var2) { // L: 28
			...
			for (var4 = var1.method1183(5) + 1; var3 < this.entries; ++var4) { // L: 30 31 34
				int var5 = var1.method1183(EnumComposition.iLog(this.entries - var3)); // L: 32
			...
			boolean var15 = var1.method1179() != 0; // L: 38

			for (var4 = 0; var4 < this.entries; ++var4) { // L: 39
				if (var15 && var1.method1179() == 0) { // L: 40
					this.lengthMap[var4] = 0;
				} else {
					this.lengthMap[var4] = var1.method1183(5) + 1; // L: 41
				}
			}
		}

		this.method1095(); // L: 44
		var3 = var1.method1183(4); // L: 45
		if (var3 > 0) { // L: 46
			float var16 = VorbisSample.float32Unpack(var1.method1183(32)); // L: 47
			float var17 = VorbisSample.float32Unpack(var1.method1183(32)); // L: 48
			var6 = var1.method1183(4) + 1; // L: 49
			boolean var7 = var1.method1179() != 0; // L: 50
			int var8;
			if (var3 == 1) { // L: 52
				var8 = mapType1QuantValues(this.entries, this.field361);
			} else {
				var8 = this.entries * this.field361; // L: 53
			}
			...
			for (var9 = 0; var9 < var8; ++var9) { // L: 55
				this.field358[var9] = var1.method1183(var6);
			}

			this.field362 = new float[this.entries][this.field361]; // L: 56
			float var10;
			int var11;
			int var12;
			if (var3 == 1) { // L: 57
				...
				for (var9 = 0; var9 < this.entries; ++var9) { // L: 71
					var10 = 0.0F; // L: 72
					var11 = var9 * this.field361; // L: 73

					for (var12 = 0; var12 < this.field361; ++var12) { // L: 74
						float var18 = (float)this.field358[var11] * var17 + var16 + var10; // L: 75
						this.field362[var9][var12] = var18; // L: 76
						if (var7) { // L: 77
							var10 = var18;
						}
	...
	int method1086(class60 var1) {
		int var2;
		for (var2 = 0; this.keys[var2] >= 0; var2 = var1.method1179() != 0 ? this.keys[var2] : var2 + 1) { // L: 152 153
		}
	...
	float[] method1087(class60 var1) {
		return this.field362[this.method1086(var1)]; // L: 158
	}
	...
	static int mapType1QuantValues(int var0, int var1) {
		int var2;
		for (var2 = (int)Math.pow((double)var0, 1.0D / (double)var1) + 1; class217.method4379(var2, var1) > var0; --var2) { // L: 16 17
		}
