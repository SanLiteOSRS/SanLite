...
public class VorbisCodebook {
	...
	VorbisCodebook() {
		class60 var1 = VorbisSample.field359; // L: 41
		var1.method1187(24); // L: 42
		this.field346 = var1.method1187(16); // L: 43
		this.entries = var1.method1187(24); // L: 44
		this.lengthMap = new int[this.entries]; // L: 45
		boolean var2 = var1.method1188() != 0; // L: 46
		int var3;
		int var4;
		int var6;
		if (var2) { // L: 47
			...
			for (var4 = var1.method1187(5) + 1; var3 < this.entries; ++var4) { // L: 49 50 53
				int var5 = var1.method1187(LoginScreenAnimation.iLog(this.entries - var3)); // L: 51
			...
			boolean var15 = var1.method1188() != 0; // L: 57

			for (var4 = 0; var4 < this.entries; ++var4) { // L: 58
				if (var15 && var1.method1188() == 0) { // L: 59
					this.lengthMap[var4] = 0;
				} else {
					this.lengthMap[var4] = var1.method1187(5) + 1; // L: 60
				}
			}
		}

		this.method1082(); // L: 63
		var3 = var1.method1187(4); // L: 64
		if (var3 > 0) { // L: 65
			float var16 = VorbisSample.float32Unpack(var1.method1187(32)); // L: 66
			float var17 = VorbisSample.float32Unpack(var1.method1187(32)); // L: 67
			var6 = var1.method1187(4) + 1; // L: 68
			boolean var7 = var1.method1188() != 0; // L: 69
			int var8;
			if (var3 == 1) { // L: 71
				var8 = mapType1QuantValues(this.entries, this.field346);
			} else {
				var8 = this.entries * this.field346; // L: 72
			}

			this.field344 = new int[var8]; // L: 73
			...
			this.field347 = new float[this.entries][this.field346]; // L: 75
			float var10;
			int var11;
			int var12;
			if (var3 == 1) { // L: 76
				for (var9 = 0; var9 < this.entries; ++var9) { // L: 77
					...
					for (var12 = 0; var12 < this.field346; ++var12) { // L: 80
						int var13 = var9 / var11 % var8; // L: 81
						float var14 = (float)this.field344[var13] * var17 + var16 + var10; // L: 82
						this.field347[var9][var12] = var14; // L: 83
						if (var7) { // L: 84
							var10 = var14;
						}
				...
				for (var9 = 0; var9 < this.entries; ++var9) { // L: 90
					var10 = 0.0F; // L: 91
					var11 = var9 * this.field346; // L: 92

					for (var12 = 0; var12 < this.field346; ++var12) { // L: 93
						float var18 = (float)this.field344[var11] * var17 + var16 + var10; // L: 94
						this.field347[var9][var12] = var18; // L: 95
						if (var7) { // L: 96
							var10 = var18;
						}
	...
	int method1080(class60 var1) {
		int var2;
		for (var2 = 0; this.keys[var2] >= 0; var2 = var1.method1188() != 0 ? this.keys[var2] : var2 + 1) { // L: 171 172
		}
	...
	float[] method1081(class60 var1) {
		return this.field347[this.method1080(var1)]; // L: 177
	}
