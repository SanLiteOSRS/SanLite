		...
		if (code != null)
		{
			...
			for (ListIterator<Instruction> iterator = ins.listIterator(); iterator.hasNext(); )
			{
				...
				else if (i instanceof GetStatic)
				{
					...
					if (method.getClassFile().getName().equals(field.getClazz().getName()) &&
							field.getType().toString().contains("Lnet/runelite/rs/api/RS"))
					{
						shadowFields.add(field.getType());

						String fieldName = field.getType().toString().replace("Lnet/runelite/rs/api/RS", "").replace(";", "");
						final Field deobTargetField = InjectUtil.findStaticField(inject,
								fieldName.substring(0, 1).toLowerCase() + fieldName.substring(1),
								null, InjectUtil.apiToDeob(inject, field.getType()));
